...
function toApiError(error: unknown): ApiError {
  ...
  if (axios.isAxiosError(error)) {
    ...
    if (!error.response) {
      return new ApiError(
        "Cannot reach the Athena API. If the backend uses a self-signed certificate, open https://localhost:8080 in this browser once and accept the certificate, then retry.",
        { statusCode: 0 },
      );
    }
...
function createClient() {
  const instance = axios.create({
    baseURL: process.env.NEXT_PUBLIC_API_URL ?? "https://localhost:8080/api/v1",
